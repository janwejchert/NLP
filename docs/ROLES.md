...
| --- | --- | --- | --- | --- |
| **Jan** *(repo owner)* | App / UI & hosting + analysis notebook | `app/streamlit_app.py`, `notebooks/analysis.ipynb`, Streamlit Cloud deploy | Build and maintain the Streamlit web UI (instruction + readback text input, verdict display); author the reproducible analysis notebook (`notebooks/build_notebook.py` → `analysis.ipynb`); manage the GitHub repo; deploy the hosted demo on Streamlit Community Cloud (`EXTRACTOR_BACKEND="hf"` + `HF_TOKEN` in Settings → Secrets) | Functional system, analysis notebook, live hosted demo link, code repository link |
